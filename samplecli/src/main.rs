...
impl RpnCalculator {
    ...
    fn eval_inner(&self, tokens: &mut Vec<&str>) -> Result<i32> {
        ...
        while let Some(token) = tokens.pop() {
            ...
            // -vオプションが指定されている場合、この時点でのトークンとスタックの状態出力
            if self.0 {
...
fn run<R: BufRead>(reader: R, verbose: bool) -> Result<()> {
    ...
    for line in reader.lines() {
        ...
        // let tokens: Vec<&str> = line.split_whitespace().collect();  // 各行をトークンに分割
        // match calc.eval_inner(&mut tokens.into_iter().rev().collect::<Vec<&str>>()) {
        //     Ok(answer) => println!("{}", answer),
        //     Err(e) => eprintln!("{:#?}", e),
        // }
    }
